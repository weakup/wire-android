...
class MessageNotificationsControllerTest extends AndroidFreeSpec { this: Suite =>
  ...
  class NewModule extends Module {
    ...
    private val notificationManager = new NotificationManagerWrapper {
      ...
      def cancel(notificationId: Int): Unit = {
        notsToCancel.mutate(_ + notificationId)
      }
    }
  ...
  def receiveNotificationsFromManyUsers(): Unit = {
    ...
    waitForOne { props =>
      props.contentTitle.exists(_.header.args == AnyRefArgs(List(3.asInstanceOf[AnyRef], "9"))) && // nine notifications from three conversations
  ...
  def receiveEphemeralAsFirstInTwoConvs(): Unit = {
    ...
    waitForOne { props =>
      props.contentTitle.exists(_.header.args == AnyRefArgs(List[AnyRef](2.asInstanceOf[AnyRef], "3"))) && // the title says that these are three notifications from two conversations
